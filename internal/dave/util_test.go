...
// copyFileWithHash copies a file from src to dst and returns the hashes.
func copyFileWithHash(dst, src string) (map[string]string, error) {
	...
	// Write data
	hw := newHashWriter(dstFile)
	if _, err = io.Copy(hw, srcFile); err != nil {
		return nil, err
	}
	return hw.Hashes(), nil
}
